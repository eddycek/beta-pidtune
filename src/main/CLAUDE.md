...
- Compact metrics: `FilterMetricsSummary` (noise floor, peaks, 128-bin spectrum, throttle spectrogram, `spectrumScaleVersion` stamp — ConvergenceDetector refuses cross-scale-version noise comparisons), `PIDMetricsSummary` (step response), `TransferFunctionMetricsSummary` (bandwidth, phase margin, dcGain, throttleBands)
